...
{
    public partial class _cb_chqoutchangecancel : UserControl
    {
            ...
            if (this._ds == null)
            {
                try
                {
                    ...
                    string __query = string.Format("select {1},{2},{3},{4},{5}"
                        + " from {0} where trans_type=2 and trans_flag=32 and  {6} {7} {8} {9}",
                        ...
                        _g.d.cb_trans_detail._remark,         //{4}
                         "(select coalesce(sum(sum_amount),0) from cb_trans_detail where trans_type=2 and trans_flag=32 ) as sum_amount",  //{5}                        
                        __where,                            //{6}
                        __getUserWhere1,                    //{7}
                        __getUserWhere2,                    //{8}
                        __orderBy);                         //{9}
        ...
        private void _showCondition()
        {
            ...
            {
                this._myCondition = new _condition_form(_enum_screen_report_cb._cb_chqoutchangecancel.ToString());
        ...
        {
            ////////////////this._myCondition.TitleName = ":: รายงานยกเลิกเปลี่ยนเช็คจ่าย ::";
            ////////////////this._myCondition.PageName = "_cb_chqoutchangecancel";
            ////////////////this._myCondition.ShowDialog();
            ////////////////_openpop = true;
            ////////////////_ds = null;
            this._showCondition();
        ...
        ////////////////////            + "(select coalesce(sum(sum_amount),0) from cb_trans_detail where trans_type=2 and trans_flag=32 )"
        ////////////////////            + " as sum_amount"
        ////////////////////            + " from cb_trans_detail where trans_type=2 and trans_flag=32 " + _getConditonResult;
        ...
        bool _view1__getObject(System.Collections.ArrayList objectList, SMLReport._report._objectType type)
        {
            if (type == SMLReport._report._objectType.Header)
            {
                ...
                _view1._addCell(__headerObject, __newColumn, true, 1, -1, SMLReport._report._cellType.String, "รายงานยกเลิกเปลี่ยนเช็คจ่าย", SMLReport._report._cellAlign.Center, _view1._fontHeader2);
