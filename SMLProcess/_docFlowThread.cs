                    ...
                    for (int __row = 0; __row < __docResult.Tables[0].Rows.Count; __row++)
                    {
                        ...
                        switch (_transType)
                        {
                            ...
                            case _g.g._transControlTypeEnum.ซื้อ_ใบสั่งซื้อ:
                                ...
                                __queryDocRefCount = "(select count(*) from (select doc_no from ic_trans as doc_ref where doc_ref.doc_ref = ic_trans.doc_no and doc_ref.trans_flag = 8 union all select doc_no from ic_trans as doc_ref_cancel where doc_ref_cancel.doc_ref = ic_trans.doc_no and doc_ref_cancel.trans_flag = 7  and doc_ref_cancel.cancel_type = 2 union all select ref_doc_no  from ic_trans_detail where ic_trans_detail.last_status = 0 and ic_trans_detail.ref_doc_no = ic_trans.doc_no and ic_trans_detail.trans_flag in (12,310)   union all select doc_no from erp_credit_approved_logs where erp_credit_approved_logs.doc_no = ic_trans.doc_no and erp_credit_approved_logs.trans_flag = erp_credit_approved_logs.trans_flag ) as doc_ref_2)";
