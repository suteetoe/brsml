...
namespace AKZOBIImport
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            string _biServerDatabaseName = "icibireportcenter";

            string _biNewServerDatabaseName = "icidtsdatacenter";

            string _biOldServerAddress = "thmrt.homeip.net:8080"; 
            string _biOldServerConfig = "SMLConfigDASHBOARD.xml"; 
            MyLib._myGlobal._databaseType _biOldServerType = MyLib._myGlobal._databaseType.MicrosoftSQL2005;
            string _biOldServerDatabaseName = "icidtsdatacenter"; // "bal";
            ...
            // start copy from old server
            MyLib._myFrameWork __myFrameWork = new MyLib._myFrameWork(_biOldServerAddress, _biOldServerConfig, _biOldServerType);
            MyLib._myFrameWork __biCenterFrameWork = new MyLib._myFrameWork(_biCenterHost, _biCenterConfig, _biCenterType);

            #region Import Ar_customer Old Server 

            string __customerOldServerFieldList = MyLib._myGlobal._fieldAndComma(
                "code",
                "code_old",
                "name_1",
                "name_2",
            ...
            DataSet _arDS = __myFrameWork._queryStream(_biOldServerDatabaseName, "select " + __customerOldServerFieldList + " from ar_customer ");
            if (_arDS != null && _arDS.Tables.Count > 0)
            {
                DataTable _arTable = _arDS.Tables[0];
                StringBuilder __queryList = new StringBuilder(MyLib._myGlobal._xmlHeader + "<node>");
                for (int __row = 0; __row < _arTable.Rows.Count; __row++)
                {
                    // pack queru and insert
                    string __valueList = MyLib._myGlobal._fieldAndComma(
                        "\'" + _arTable.Rows[__row]["code"].ToString() + "\'",
                        "\'" + _arTable.Rows[__row]["code_old"].ToString() + "\'",
                        "\'" + _arTable.Rows[__row]["name_1"].ToString() + "\'",
                        "\'" + _arTable.Rows[__row]["name_2"].ToString() + "\'",
                        "\'" + _arTable.Rows[__row]["name_eng_1"].ToString() + "\'",
                        "\'" + _arTable.Rows[__row]["name_eng_2"].ToString() + "\'",
                        "\'" + _arTable.Rows[__row]["address"].ToString() + "\'",
                        "\'" + _arTable.Rows[__row]["address_eng"].ToString() + "\'",
                        "\'" + _arTable.Rows[__row]["tambon"].ToString() + "\'",
                        "\'" + _arTable.Rows[__row]["amper"].ToString() + "\'",
                        "\'" + _arTable.Rows[__row]["province"].ToString() + "\'",
                        "\'" + _arTable.Rows[__row]["zip_code"].ToString() + "\'",
                        "\'" + _arTable.Rows[__row]["telephone"].ToString() + "\'",
                        "\'" + _arTable.Rows[__row]["fax"].ToString() + "\'",
                        "\'" + _arTable.Rows[__row]["email"].ToString() + "\'",
                        "\'" + _arTable.Rows[__row]["website"].ToString() + "\'",
                        "\'" + _arTable.Rows[__row]["description"].ToString() + "\'",
                        _getDateStr(_arTable.Rows[__row]["birth_day"].ToString()),
                        "\'" + _arTable.Rows[__row]["ar_type"].ToString() + "\'",
                        "\'" + _arTable.Rows[__row]["remark"].ToString() + "\'",
                        "\'" + _arTable.Rows[__row]["status"].ToString() + "\'",
                        "\'" + _arTable.Rows[__row]["guid_code"].ToString() + "\'",
                        "\'" + _arTable.Rows[__row]["ar_status"].ToString() + "\'",
                        "\'" + _arTable.Rows[__row]["guid"].ToString() + "\'",
                        "\'" + _arTable.Rows[__row]["agencode"].ToString() + "\'",
                        "\'" + _arTable.Rows[__row]["databasename"].ToString() + "\'",
                        "\'" + _arTable.Rows[__row]["datetimemanage"].ToString() + "\'",
                        "\'" + _arTable.Rows[__row]["is_lock_record"].ToString() + "\'",
                        "\'" + _arTable.Rows[__row]["ignore_sync"].ToString() + "\'",
                        "\'" + _arTable.Rows[__row]["doc_format_code"].ToString() + "\'",
                        "\'" + _arTable.Rows[__row]["prefixname"].ToString() + "\'",
                        "\'" + _arTable.Rows[__row]["first_name"].ToString() + "\'",
                        "\'" + _arTable.Rows[__row]["last_name"].ToString() + "\'",
                        "\'" + _arTable.Rows[__row]["price_level"].ToString() + "\'",
                        "\'" + _arTable.Rows[__row]["point_balance"].ToString() + "\'"
                        );

                    string __query = "insert into ar_customer (" + __customerFieldList + ") values(" + __valueList + ")";

                    __queryList.Append(MyLib._myUtil._convertTextToXmlForQuery(__query));
                }
                __queryList.Append("</node>");

                string __debug = __queryList.ToString();
            ...
            DataTable __countTable =  __myFrameWork._query(_biOldServerDatabaseName, "select count(*) as xcount from ic_trans ").Tables[0];
            if (__countTable.Rows.Count > 0 && decimal.Parse(__countTable.Rows[0][0].ToString()) > 0)
            {
                string __str = __countTable.Rows[0][0].ToString();
                int __maxRecord = int.Parse(__countTable.Rows[0][0].ToString());
                decimal __maxLoop = Math.Ceiling(decimal.Parse(__countTable.Rows[0][0].ToString()) / 5000);
                ...
                for (int __loop = 0; __loop < __maxLoop; __loop++)
                {
                    ...
                    DataSet __icTransDataSet = __myFrameWork._queryLimit(_biOldServerDatabaseName, "select count(*) as xcount from ic_trans ", "select " + __icTransOldServerFieldList + " from ic_trans order by roworder", __offset, 5000, __maxRecord).detail;
                    if (__icTransDataSet != null && __icTransDataSet.Tables.Count > 0)
                    {
                        DataTable __icTransTable = __icTransDataSet.Tables[0];
                        StringBuilder __queryList = new StringBuilder(MyLib._myGlobal._xmlHeader + "<node>");
                        for (int __row = 0; __row < __icTransTable.Rows.Count; __row++)
                        {
                            // pack queru and insert
                            string __valueList = MyLib._myGlobal._fieldAndComma(
                                "\'" + __icTransTable.Rows[__row]["trans_type"].ToString() + "\'",
                                "\'" + __icTransTable.Rows[__row]["trans_flag"].ToString() + "\'",
                                "\'" + __icTransTable.Rows[__row]["doc_group"].ToString() + "\'",
                                "\'" + __icTransTable.Rows[__row]["doc_date"].ToString() + "\'",
                                "\'" + __icTransTable.Rows[__row]["doc_no"].ToString() + "\'",

                                "\'" + __icTransTable.Rows[__row]["doc_ref"].ToString() + "\'",
                                _getDateStr(__icTransTable.Rows[__row]["doc_ref_date"].ToString()),
                                "\'" + __icTransTable.Rows[__row]["tax_doc_no"].ToString() + "\'",
                                _getDateStr(__icTransTable.Rows[__row]["tax_doc_date"].ToString()),
                                "\'" + __icTransTable.Rows[__row]["inquiry_type"].ToString() + "\'",

                                "\'" + __icTransTable.Rows[__row]["vat_type"].ToString() + "\'",
                                "\'" + __icTransTable.Rows[__row]["cust_code"].ToString() + "\'",
                                "\'" + __icTransTable.Rows[__row]["contactor"].ToString() + "\'",
                                "\'" + __icTransTable.Rows[__row]["branch_code"].ToString() + "\'",
                                "\'" + __icTransTable.Rows[__row]["project_code"].ToString() + "\'",

                                "\'" + __icTransTable.Rows[__row]["side_code"].ToString() + "\'",
                                "\'" + __icTransTable.Rows[__row]["department_code"].ToString() + "\'",
                                "\'" + __icTransTable.Rows[__row]["sale_area_code"].ToString() + "\'",
                                "\'" + __icTransTable.Rows[__row]["sale_code"].ToString() + "\'",
                                "\'" + __icTransTable.Rows[__row]["send_type"].ToString() + "\'",

                                "\'" + __icTransTable.Rows[__row]["send_day"].ToString() + "\'",
                                _getDateStr(__icTransTable.Rows[__row]["send_date"].ToString()),
                                "\'" + __icTransTable.Rows[__row]["expire_day"].ToString() + "\'",
                                _getDateStr(__icTransTable.Rows[__row]["expire_date"].ToString()),
                                "\'" + __icTransTable.Rows[__row]["credit_day"].ToString() + "\'",

                                _getDateStr(__icTransTable.Rows[__row]["due_date"].ToString()),
                                "\'" + __icTransTable.Rows[__row]["answer_type"].ToString() + "\'",
                                "\'" + __icTransTable.Rows[__row]["transport_value"].ToString() + "\'",
                                "\'" + __icTransTable.Rows[__row]["discount_word"].ToString() + "\'",
                                "\'" + __icTransTable.Rows[__row]["vat_rate"].ToString() + "\'",

                                "\'" + __icTransTable.Rows[__row]["currency_code"].ToString() + "\'",
                                "\'" + __icTransTable.Rows[__row]["exchange_rate"].ToString() + "\'",
                                "\'" + __icTransTable.Rows[__row]["total_cost"].ToString() + "\'",
                                "\'" + __icTransTable.Rows[__row]["total_value"].ToString() + "\'",
                                "\'" + __icTransTable.Rows[__row]["total_discount"].ToString() + "\'",

                                "\'" + __icTransTable.Rows[__row]["total_vat_value"].ToString() + "\'",
                                "\'" + __icTransTable.Rows[__row]["total_after_vat"].ToString() + "\'",
                                "\'" + __icTransTable.Rows[__row]["total_except_vat"].ToString() + "\'",
                                "\'" + __icTransTable.Rows[__row]["total_amount"].ToString() + "\'",
                                "\'" + __icTransTable.Rows[__row]["balance_amount"].ToString() + "\'",

                                "\'" + __icTransTable.Rows[__row]["adjust_reason"].ToString() + "\'",
                                "\'" + __icTransTable.Rows[__row]["approve_code"].ToString() + "\'",
                                _getDateStr(__icTransTable.Rows[__row]["approve_date"].ToString()),
                                "\'" + __icTransTable.Rows[__row]["remark"].ToString() + "\'",
                                "\'" + __icTransTable.Rows[__row]["status"].ToString() + "\'",

                                "\'" + __icTransTable.Rows[__row]["guid_code"].ToString() + "\'",
                                "\'" + __icTransTable.Rows[__row]["user_request"].ToString() + "\'",
                                "\'" + __icTransTable.Rows[__row]["send_mail"].ToString() + "\'",
                                "\'" + __icTransTable.Rows[__row]["send_sms"].ToString() + "\'",
                                "\'" + __icTransTable.Rows[__row]["doc_ref_trans"].ToString() + "\'",

                                "\'" + __icTransTable.Rows[__row]["credit_date"].ToString() + "\'",
                                "\'" + __icTransTable.Rows[__row]["total_before_vat"].ToString() + "\'",
                                "\'" + __icTransTable.Rows[__row]["doc_time"].ToString() + "\'",
                                "\'" + __icTransTable.Rows[__row]["allocate_code"].ToString() + "\'",
                                "\'" + __icTransTable.Rows[__row]["job_code"].ToString() + "\'",

                                _getDateStr(__icTransTable.Rows[__row]["delivery_date"].ToString()),
                                "\'" + __icTransTable.Rows[__row]["pay_amount"].ToString() + "\'",
                                "\'" + __icTransTable.Rows[__row]["money"].ToString() + "\'",
                                "\'" + __icTransTable.Rows[__row]["sum_pay_money_diff"].ToString() + "\'",
                                "\'" + __icTransTable.Rows[__row]["currency_money"].ToString() + "\'",

                                "\'" + __icTransTable.Rows[__row]["total_debt_amount"].ToString() + "\'",
                                "\'" + __icTransTable.Rows[__row]["cancel_type"].ToString() + "\'",
                                "\'" + __icTransTable.Rows[__row]["last_status"].ToString() + "\'",
                                "\'" + __icTransTable.Rows[__row]["used_status"].ToString() + "\'",
                                "\'" + __icTransTable.Rows[__row]["doc_success"].ToString() + "\'",

                                "\'" + __icTransTable.Rows[__row]["total_manual"].ToString() + "\'",
                                "\'" + __icTransTable.Rows[__row]["on_hold"].ToString() + "\'",
                                "\'" + __icTransTable.Rows[__row]["extra_word"].ToString() + "\'",
                                "\'" + __icTransTable.Rows[__row]["transport_code"].ToString() + "\'",
                                "\'" + __icTransTable.Rows[__row]["want_day"].ToString() + "\'",

                                "null", //_getDateStr(__icTransTable.Rows[__row]["want_date"].ToString()),
                                "\'" + __icTransTable.Rows[__row]["deposit_day"].ToString() + "\'",
                                _getDateStr(__icTransTable.Rows[__row]["deposit_date"].ToString()),
                                "\'" + __icTransTable.Rows[__row]["sale_group"].ToString() + "\'",
                                "\'" + __icTransTable.Rows[__row]["not_approve_1"].ToString() + "\'",

                                "\'" + __icTransTable.Rows[__row]["user_approve"].ToString() + "\'",
                                "\'" + __icTransTable.Rows[__row]["approve_status"].ToString() + "\'",
                                "\'" + __icTransTable.Rows[__row]["expire_status"].ToString() + "\'",
                                "\'" + __icTransTable.Rows[__row]["advance_amount"].ToString() + "\'",
                                "\'" + __icTransTable.Rows[__row]["doc_format_code"].ToString() + "\'",

                                "\'" + __icTransTable.Rows[__row]["used_status_2"].ToString() + "\'",
                                "\'" + __icTransTable.Rows[__row]["remark_2"].ToString() + "\'",
                                "\'" + __icTransTable.Rows[__row]["remark_3"].ToString() + "\'",
                                "\'" + __icTransTable.Rows[__row]["ref_amount"].ToString() + "\'",
                                "\'" + __icTransTable.Rows[__row]["ref_new_amount"].ToString() + "\'",

                                "\'" + __icTransTable.Rows[__row]["ref_diff"].ToString() + "\'",
                                "\'" + __icTransTable.Rows[__row]["sum_point"].ToString() + "\'",
                                "\'" + __icTransTable.Rows[__row]["cashier_code"].ToString() + "\'",
                                "\'" + __icTransTable.Rows[__row]["is_pos"].ToString() + "\'",
                                "\'" + __icTransTable.Rows[__row]["pos_id"].ToString() + "\'",

                                "\'" + __icTransTable.Rows[__row]["member_code"].ToString() + "\'",
                                "\'" + __icTransTable.Rows[__row]["doc_no_guid"].ToString() + "\'",
                                "\'" + __icTransTable.Rows[__row]["pos_bill_type"].ToString() + "\'",
                                "\'" + __icTransTable.Rows[__row]["pos_bill_change"].ToString() + "\'",
                                "\'" + __icTransTable.Rows[__row]["doc_type"].ToString() + "\'",
                                "\'" + __icTransTable.Rows[__row]["auto_create"].ToString() + "\'",
                                "\'" + convertAgenCode(__icTransTable.Rows[__row]["agencode"].ToString()) + "\'"
                                );

                            string __query = "insert into ic_trans (" + __icTransFieldList + ") values(" + __valueList + ")";
                        ...
                        __icTransTable.Dispose();
                    }
                    __icTransDataSet.Dispose();
                }
            }
            #endregion
