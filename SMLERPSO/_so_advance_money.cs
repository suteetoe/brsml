...
namespace SMLERPSO
{
    public partial class _so_advance_money : UserControl
    {
        public _so_advance_money()
        {
            InitializeComponent();
            this._po_so_deposit_control1._screen_code = "SD";
        ...
        void _printData()
        {
            ...
            {
                // toe ดัก print ของ smlcolorstore
                if (MyLib._myGlobal._isVersionEnum == MyLib._myGlobal._versionType.SMLColorStore)
                {
                    try
                    {
                        ...
                        {
                            SMLInventoryControl._formSaleOrderPreview __showPrintDialog = new SMLInventoryControl._formSaleOrderPreview(SMLInventoryControl._gForm._formEnum.ขาย_รับเงินล่วงหน้า.ToString(), __getDocNo, __getDocDate, __getArCode);
