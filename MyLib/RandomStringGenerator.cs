...
namespace MyLib
{
    public class RandomStringGenerator
    {
        ...
        {
            return NextString(length, lowerCase, upperCase, numbers, symbols, true);
        }

        public virtual string NextString(int length, bool lowerCase, bool upperCase, bool numbers, bool symbols, bool canStartZero)
        {
            ...
            for (int i = 0; i < charArray.Length; i++)
            {
                ...
                if (i == 0 && canStartZero == false)
                {                    
                    string __newCharPool = string.Empty;
                    __newCharPool = charPool.Replace("0", string.Empty); ;

                    int index = r.Next(0, __newCharPool.Length);

                    //Set it to the output character array
                    charArray[i] = __newCharPool[index];
                }
                else
                {
