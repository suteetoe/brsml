...
namespace SMLERPAR
{
    public partial class _ar_pay_bill : UserControl
    {
        public _ar_pay_bill(string menuName)
        {
            InitializeComponent();
            ...
            if (this._ar_ap_trans1._printFormAfterSave == false)
            {
                try
                {
                    ...
                    if (MyLib._myGlobal._isVersionEnum == MyLib._myGlobal._versionType.SMLColorStore)
                    {
                        ...
                        {
                            SMLInventoryControl._formSaleOrderPreview __showPrintDialog = new SMLInventoryControl._formSaleOrderPreview(SMLInventoryControl._gForm._formEnum.ลูกหนี้_ใบวางบิล.ToString(), __getDocNo, __getDocDate, __getArCode);
