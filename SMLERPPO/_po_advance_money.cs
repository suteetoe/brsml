...
namespace SMLERPPO
{
    public partial class _po_advance_money : UserControl
    {
        // จ่ายเงินล่วงหน้า
        public _po_advance_money(string screenName)
        {
            InitializeComponent();
            this._po_so_deposit_control1._screen_code = "PD";
            ...
            this._po_so_deposit_control1._afterSave += _po_so_deposit_control1__afterSave;
        }

        void _po_so_deposit_control1__afterSave()
        {
            try
            {
                _printData();
        ...
        void _printData()
        {
            if (this._po_so_deposit_control1._printFormAfterSave == false)
            {
                try
                {
                    if (MyLib._myGlobal._isVersionEnum == MyLib._myGlobal._versionType.SMLColorStore)
                    {
                        ...
                        {
                            SMLInventoryControl._formSaleOrderPreview __showPrintDialog = new SMLInventoryControl._formSaleOrderPreview(SMLInventoryControl._gForm._formEnum.ซื้อ_จ่ายเงินมัดจำหรือเงินล่วงหน้า.ToString(), __getDocNo, __getDocDate, __getArCode);
