...
namespace SMLERPAP
{
    public partial class _ap_pay_bill : UserControl
    {
        public _ap_pay_bill(string menuName)
        {
            InitializeComponent();
            this._ar_ap_trans1._screenTop._screen_code = "DD";
            ...
            if (this._ar_ap_trans1._printFormAfterSave == false)
            {
                try
                {
                    ...
                    if (MyLib._myGlobal._isVersionEnum == MyLib._myGlobal._versionType.SMLColorStore)
                    {
                        ...
                        {
                            SMLInventoryControl._formSaleOrderPreview __showPrintDialog = new SMLInventoryControl._formSaleOrderPreview(SMLInventoryControl._gForm._formEnum.เจ้าหนี้_ใบรับวางบิล.ToString(), __getDocNo, __getDocDate, __getArCode);
