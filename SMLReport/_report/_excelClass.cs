...
namespace SMLReport._report
{
    public class _excelClass
    {
        ...
        public string _createExcelFile(string fileName, Boolean createGuid)
        {
            string __xmlFileName = (createGuid) ? fileName + Guid.NewGuid().ToString("N") + ".xml" : fileName;
                ...
                for (int __column = 0; __column < __getRow._cell.Count; __column++)
                {
                    ...
                    //else
                    {
                        ...
                        if (__getCell._value != null)
                        {
                            ...
                            {
                                case SMLReport._report._cellType.String: __dataType = "String"; break;
                                case SMLReport._report._cellType.Number: __dataType = "Number"; break;
                                case SMLReport._report._cellType.DateTime: __dataType = "DateTime"; break;
                            ...
                            if (__getCell._dataType == _cellType.Number && __getData.Length > 14)
                            {
                                try
                                {
                                    double __numberCheck = double.Parse(__getData);
                                    if (__numberCheck > 100000000000000)
                                    {
                                        __xmlString.Append(string.Format("{0:0.##############E+000}", __numberCheck)); //__value.ToString(textBox2.Text);e

                                    }
                                    else
                                    {
                                        __xmlString.Append(__numberCheck.ToString());
                                    }
                                }
                                catch
                                {
                                    __xmlString.Append(MyLib._myUtil._convertTextToXml(__getData));
                                }
                            }
                            else
                                __xmlString.Append(MyLib._myUtil._convertTextToXml(__getData));
        ...
        {
            ((_cellClass)((_rowClass)_rowData[row])._cell[column])._index = this._currentColumn + 1; //toe
            //((_cellClass)((_rowClass)_rowData[row])._cell[column])._index = this._currentColumn + columnCount;
            ((_cellClass)((_rowClass)_rowData[row])._cell[column])._value = value;
            ((_cellClass)((_rowClass)_rowData[row])._cell[column])._styleName = styleName;
            this._currentColumn = this._currentColumn + columnCount;
        ...
        public void _cellValue(int row, int column, object value, string styleName, SMLReport._report._cellType dataType)
        {
            // toe ดัก value number เป็นค่าติดลบ
            if (dataType == _cellType.Number && value.ToString().Length > 0 && value.ToString().Substring(0, 1).Equals("(") == true && value.ToString().Substring(value.ToString().Length - 1, 1).Equals(")") == true)
            {
                value = MyLib._myUtil._moneyReFormat(value.ToString(), styleName);
            }
        ...
        public void _cellValue(int row, int column, object value, string styleName, int columnCount, SMLReport._report._cellType dataType)
        {
        ...
        public void _cellValue(int row, int column, object value, string styleName, int columnCount, SMLReport._report._cellType dataType, int index)
        {
    ...
    public class _cellClass
    {
        ...
        /// </summary>
        public SMLReport._report._cellType _dataType = SMLReport._report._cellType.String;
        /// <summary>
        /// Border มาตฐาน
        /// </summary>
