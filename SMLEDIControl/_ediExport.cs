            ...
            if (__docList.Length > 0)
            {
                ...
                string __queryGetDataTrans = "select " +
                    MyLib._myGlobal._fieldAndComma(
                        ...
                        "(select ship_code from ap_ar_transport_label where ar_ap_type = 1 and cust_code = ic_trans.cust_code and name_1 = (select transport_name from ic_trans_shipment where ic_trans_shipment.doc_no = ic_trans.doc_no and ic_trans_shipment.trans_flag = ic_trans.trans_flag) limit 1) as ship_code",
